...
router.post('/:listId', async (req, res) => {
  const { listId } = req.params;
  const { tmdb_id, title, year, poster_url, rating, overview, director, genres, added_by } = req.body;
  try {
    const movie = await pool.query(
      `INSERT INTO movies (imdb_id, title, year, poster_url, imdb_rating)
       VALUES ($1, $2, $3, $4, $5)
       ON CONFLICT (imdb_id) DO UPDATE SET title = EXCLUDED.title
       RETURNING *`,
      [`tmdb_${tmdb_id}`, title, year, poster_url, rating]
    );
    await pool.query(
